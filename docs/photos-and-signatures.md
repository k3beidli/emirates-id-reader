...
[V1/V2 compatibility](card-generations.md). Generation is not a guarantee either
way, so inspect the status rather than the generation label.

## Related

- [Application integration](integration.md) for displaying images in a UI
- [Errors and read statuses](error-handling.md) for the full status model
- [Security and access boundaries](security.md) for what is never requested
