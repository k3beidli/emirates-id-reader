...
| --- | --- | --- | --- |
| `name()` | `Option<&str>` | Stored English name, falling back to Arabic | [Names](Names) |
| `name_in(Language)` | `Option<&str>` | Stored name in one language; no fallback | [Names](Names) |
| `name_components_in(Language)` | `impl Iterator<Item = &str>` | Stored components in card order | [Names](Names) |
| `photo()` | `Option<&[u8]>` | JPEG photograph payload | [Photos](Photos-And-Signatures) |
| `signature()` | `Option<&[u8]>` | Signature payload; format may vary | [Photos](Photos-And-Signatures) |
| `id_number()` | `&str` | Required 15-digit ID | [Codes](Codes-And-Identifiers) |
| `card_number()` | `&str` | Required card number | [Codes](Codes-And-Identifiers) |
| `gender_code()` | `Option<&str>` | Stored gender code | [Codes](Codes-And-Identifiers) |
| `nationality_code()` | `Option<&str>` | Stored nationality code | [Codes](Codes-And-Identifiers) |
| `nationality_in(Language)` | `Option<&str>` | Nationality description; no fallback | [Data model](Data-Model) |
| `date_of_birth()` | `Option<&str>` | `YYYY-MM-DD` | [Dates](Dates) |
| `issue_date()` | `Option<&str>` | `YYYY-MM-DD` | [Dates](Dates) |
| `expiry_date()` | `Option<&str>` | `YYYY-MM-DD` | [Dates](Dates) |
...
| Method | Return type | Meaning | Guide |
| --- | --- | --- | --- |
| `formatted_name()` | `Option<String>` | Display name, English with Arabic fallback | [Names](Names) |
| `formatted_name_in(Language)` | `Option<String>` | Display name in one language | [Names](Names) |
...
[the runnable example](Names#formatting-without-a-card).
