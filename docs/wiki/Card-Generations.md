...
The names **V1** and **V2** in this library refer to the chip generations used by
...
V1 card in the earlier hardware results returned extended public fields plus its
...
published. The library matches the byte sequence and does not rely on the label.

An unrecognised ATR is reported as `CardGeneration::Unknown`; the library still
...
Full details in [sources and acknowledgments](Sources).
...
outside this library's public-data scope. The library never attempts fingerprint
...
request its V2-only extension API from a V1 card. This Rust library does not call
...
| Library field/group | V1 | V2 | Notes |
| --- | --- | --- | --- |
| `id_number` | Yes | Yes | Required by the library |
| `card_number` | Yes | Yes | Required by the library |
...
| Arabic/English full names | Yes | Yes | Comma-separated name components; `formatted_name()` joins them for display |
...
| mother's Arabic/English name | First name | Full name | Exposed through the same library fields |
| `holder_signature_image` | No | Yes | Optional file; also observed on V1 |
...
The earlier implementation reported successful V1 and V2 reads with an HID
OMNIKEY 3121 on Windows, including extended fields on the tested V1 card.
Those results do not establish hardware support for the current library revision.
