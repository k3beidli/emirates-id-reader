...
[V1 specification](sources.md#fields-stored-in-uae-id-card-v1) also lists `X`;
...
nationality all have code fields on [`identity()` or `extended()`](field-reference.md).
...
- [Data model and formatting](data-model.md) for what the library does and does not
  interpret
- [Field reference](field-reference.md) for every code field and its type
- [V1/V2 compatibility](card-generations.md) for which descriptions each
  generation stores
